...
const INTERNET_RADIO_TAG = { id: 'navidrome__internet_radio', title: '网络电台' };
...
function getUserAgent() {
    if (typeof navigator !== 'undefined' && navigator.userAgent) {
        if (/Android/i.test(navigator.userAgent)) return 'MusicFree (Android)';
        if (/Windows/i.test(navigator.userAgent)) return 'MusicFree (Windows NT)';
        if (/Linux/i.test(navigator.userAgent)) return 'MusicFree (Linux)';
        if (/Macintosh|MacIntel|MacPPC|Mac OS|iPhone|iPad|iPod/i.test(navigator.userAgent)) return 'MusicFree (Macintosh)';
        return 'MusicFree/Web';
    }
    if (typeof process !== 'undefined' && process.platform) {
        if (process.platform === 'win32') return 'MusicFree (Windows NT)';
        if (process.platform === 'android') return 'MusicFree (Android)';
        if (process.platform === 'linux') return 'MusicFree (Linux)';
        if (process.platform === 'darwin') return 'MusicFree (Macintosh)';
...
function formatInternetRadioStation(station) {
    return {
        id: `radio-${station.id}`,
        title: station.name || "未知电台",
        artist: '网络电台',
        album: station.homePageUrl || '网络电台',
        artwork: 'https://p1.music.126.net/aXUPgImt8hhf4cMUZEjP4g==/109951165611417794.jpg',
        url: station.streamUrl,
        duration: 0,
        _source: 'navidrome_internet_radio',
    };
}
...
async function getMusicSheetInfoApi(sheetItem, page) {
    ...
    if (sheetItem.id === INTERNET_RADIO_TAG.id) {
        return await getInternetRadioStationsApi(page);
    }
    if (page > 1) {
    ...
    try {
        const response = await httpGetNative(`playlist/${sheetItem.id}/tracks`, { _start: 0, _end: 10000 });
        ...
        return {
            isEnd: true,
            musicList: tracks.map(formatPlaylistMusicItemNative),
            sheetItem: supplementarySheetData
        };

    } catch (e) {
        console.error(`获取歌单 ${sheetItem.id} 内容失败:`, e);
        return { isEnd: true, musicList: [] };
    }
}
...
async function getInternetRadioStationsApi(page) {
    if (page > 1) {
        return { isEnd: true, musicList: [] };
    }
    try {
        const response = await httpGetNative('radio', { _start: 0, _end: 200, _sort: 'name', _order: 'ASC' });
        const stations = response.data ?? [];
        return {
            isEnd: true,
            musicList: stations.map(formatInternetRadioStation),
            sheetItem: page === 1 ? { id: INTERNET_RADIO_TAG.id, title: INTERNET_RADIO_TAG.title, description: "服务器上的所有网络电台" } : undefined
        };
    } catch (e) {
        console.error("获取网络电台失败:", e);
        return { isEnd: true, musicList: [] };
    }
}
...
async function getMediaSourceApi(musicItem, quality) {
    if (musicItem._source === 'navidrome_internet_radio' && musicItem.url) {
...
async function getRecommendSheetsByTagApi(tag, page, username) {
    ...
    if (tag.id === '') {
        ...
        if (page === 1) {
            ...
            const radioSheet = {
                id: INTERNET_RADIO_TAG.id,
                title: INTERNET_RADIO_TAG.title,
                artist: username || "Navidrome",
                artwork: 'https://p1.music.126.net/aXUPgImt8hhf4cMUZEjP4g==/109951165611417794.jpg',
                description: "浏览服务器上的所有网络电台"
            };
            returnData.unshift(radioSheet);
...
module.exports = {
    ...
    version: "2.7.4",
    author: 'Ckryin',
    srcUrl: "https://gitee.com/Rrance/WP/raw/master/音海拾贝.js",
    ...
    async syncMusicSheet(sheetItem) {
        if (!sheetItem || !sheetItem.title) return false;
        try {
            const songIds = [];
            for (const music of sheetItem.musicList) {
                let id = null;
                // 1. 如果是 Navidrome 的歌曲，直接使用 ID
                if (music._source === 'navidrome' || music._source === 'navidrome_qq_artwork' || music._source === 'navidrome_ncm_artwork') {
                    id = music.id;
                }
                // 2. 如果不是，尝试搜索匹配
                else {
                    ...
                    try {
                        const searchResult = await searchMusic(query, 1, 5);
                        const data = searchResult.data || [];
                        // 简单匹配：标题和歌手相同
                        const match = data.find(item =>
                            item.title === music.title &&
                            (item.artist === music.artist || !music.artist)
                        );
                        if (match) {
                            id = match.id;
                        }
            ...
            // 3. 检查是否存在同名歌单，存在则删除 Ensure exact sync
            const playlistName = sheetItem.title;
            let existingPlaylistId = null;
            try {
                const playlistsData = await httpGet('getPlaylists', {});
                const playlists = playlistsData?.['subsonic-response']?.playlists?.playlist ?? [];
            ...
            // 4. 创建新歌单
            const newPlaylistId = await createNavidromePlaylistApi(playlistName);
            if (newPlaylistId) {
                // 5. 添加歌曲
                return await addSongsToNavidromePlaylistApi(newPlaylistId, songIds);
            }
            return false;
