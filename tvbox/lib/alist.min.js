...
function get_drives(name) {
	...
	if (settings.v3 == null) {
		settings.v3 = false;
		const data = http.get(server + "/api/public/settings", {
			headers: headers
		}).json().data;
	...
	drives.forEach(item => {
		...
		__drives[item.name] = {
			...
			getFile(path) {
				...
				if (playRaw === 1) {
					try {
						const res = http.post(this.server + this.api.file, {
							data: this.getParams(path),
							headers: this.headers
						}).json();
...
function getAll(otid, tid, drives, path) {
	try {
		...
		let list = [{
			...
			vod_id: "no_data",
			vod_remarks: "不要点,会崩的",
			vod_pic: "https://ghproxy.net/https://raw.githubusercontent.com/hjdhnx/dr_py/main/static/img/404.jpg",
			vod_actor: e.message,
			vod_director: tid,
			vod_content: otid
		}];
